...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  ...
  setupDebugMessenger();  // enable validation layers
  createSurface();  // create a surface that relies on GLFW
  pickPhysicalDevice(); // pick the physical device app will be using (e.g. gpu 0)
  createLogicalDevice();  // describes what features of the physical device we want to use
  createCommandPool(); // will help with command buffer allocation
}
...
#ifdef __APPLE__
  // Required for MoltenVK (Vulkan on macOS via Metal)
  extensions.push_back(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME);
  // Required by VK_KHR_portability_subset (device extension dependency)
